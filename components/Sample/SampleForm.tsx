...
interface SampleFormProps {
  ...
  accordionData: any[];
  selectedSamples: string[]; // ⭐ NOW STRING[]
  onRemoveSample: () => void;
}
...
export default function SampleForm({
  ...
  // ⭐ Filter selected by uniqueId
  const selectedProducts = allProducts.filter((p) =>
    selectedSamples.includes(p.uniqueId) // 🔥 FIXED
  );
  ...
  return (
    ...
    >
      {/* ⭐ Selected sample preview images */}
      {hasSamples && (
        <div className="flex gap-5 md:mb-10 mb-6">
          {selectedProducts.map((p, index) => (
            <div key={p.uniqueId} className="relative w-20 h-20">
      ...
      <div className="pt-6 space-y-4">
        ...
        <label className="flex items-start gap-2 text-base font-normal text-description font-Satoshi">
          <input
            ...
            onChange={onChange}
          />
          Yes, I would like to get quotes from recommended fitters near me.
        </label>
        ...
        <label className="flex items-start gap-2 text-base font-normal text-description font-Satoshi my-4">
          <input
            ...
            required
          />
          Add additional samples of composite decking at 3.6m length that match
          my selected choices — FREE.
        </label>
        ...
        <label className="flex items-start gap-2 text-base font-normal text-description font-Satoshi">
          ...
          communicate regarding my order and marketing (as per T&Cs).
        </label>
      </div>
